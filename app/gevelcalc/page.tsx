...
function ToggleSwitch({
  ...
  return (
    ...
    >
      ...
      <span
        className={`relative h-6 w-11 rounded-full border border-black ${
          checked ? "bg-black" : "bg-white"
        }`}
      >
        <span
          className={`absolute top-1 h-4 w-4 rounded-full transition ${
            checked ? "left-6 bg-white" : "left-1 bg-black"
          }`}
        />
      </span>
    </button>
...
export default function GevelCalcPage() {
  // #State: zijdes en koppelingen
  const [sides, setSides] = useState<Side[]>([createSide(0)]);
  const [frontBackSame, setFrontBackSame] = useState(false);
  const [leftRightSame, setLeftRightSame] = useState(false);

  // #State: productkeuze en richting
  const [selectedProductId, setSelectedProductId] = useState("");
  const [orientation, setOrientation] = useState<Orientation>("horizontal");

  // #Gekozen product uit productCatalog.ts
  const selectedProduct = products.find(
    (product) => product.id === selectedProductId
  );

  // #Helper: gekoppelde breedte ophalen
  ...
  // #Helper: gekoppelde hoogte ophalen
  ...
  // #Calculator: bruto oppervlak per zijde
  ...
  // #Calculator: totaal openingen per zijde
  function openingsM2(side: Side) {
    return round2(
      side.openings.reduce((sum, opening) => {
        return (
          sum +
          (toNumber(opening.width) / 100) *
            (toNumber(opening.height) / 100) *
            toNumber(opening.count)
        );
      }, 0)
    );
  }

  // #Calculator: netto oppervlak per zijde
  ...
  function updateSide(sideId: string, updater: (side: Side) => Side) {
    setSides((prev) =>
      prev.map((side) => (side.id === sideId ? updater(side) : side))
    );
  }
  ...
  // #Foto uploaden per zijde
  function handleImageUpload(sideId: string, file: File | null) {
  ...
  // #Totaaloverzicht: bruto, openingen en netto
  ...
  // #Materiaalberekening: panelen, prijs en profielregels
    ...
    <main className="min-h-screen bg-[#f6f4ef] p-4 pb-28 text-black md:p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-2xl border border-black bg-white p-6 text-center">
          ...
          <p className="mt-2">
            Bereken bruto oppervlak, openingen, netto geveloppervlak, panelen en prijs.
          </p>
        </section>
        ...
        <section className="rounded-2xl border border-black bg-white p-4">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {sides.length >= 2 && (
              <ToggleSwitch
                checked={frontBackSame}
                onChange={setFrontBackSame}
                label="Voorzijde en achterzijde hebben dezelfde afmetingen"
              />
            )}

            {sides.length >= 4 && (
              <ToggleSwitch
                checked={leftRightSame}
                onChange={setLeftRightSame}
                label="Linkerzijde en rechterzijde hebben dezelfde afmetingen"
              />
            )}
          </div>
        </section>
        ...
        {sides.map((side, index) => {
          const isLinked =
            (index === 1 && frontBackSame) || (index === 3 && leftRightSame);

          return (
            <section
              key={side.id}
              className="rounded-2xl border border-black bg-white p-4 md:p-6"
            >
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  {isLinked && (
                    <p className="text-sm">
                      Afmetingen worden automatisch overgenomen.
                    </p>
                  )}
                </div>
              ...
              <div className="mt-5 rounded-2xl border-2 border-dashed border-black p-4 text-center">
                <p className="mb-3 text-sm font-semibold">
                  Upload foto van deze zijde
                </p>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) =>
                    handleImageUpload(side.id, e.target.files?.[0] ?? null)
                  }
                />
                ...
                {side.previewUrl && (
                  <img
                    src={side.previewUrl}
                    alt={side.name}
                    className="mt-4 max-h-[320px] w-full rounded-xl object-contain"
                  />
                )}
              </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <ToggleSwitch
                  ...
                  onChange={(checked) => setHasWindows(side.id, checked)}
                  label="Deze zijde heeft kozijnen"
                />

                <ToggleSwitch
                  checked={side.hasDoors === "yes"}
                  onChange={(checked) => setHasDoors(side.id, checked)}
              ...
              {side.openings.length > 0 && (
                <div className="mt-5 space-y-4 rounded-2xl border border-black p-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button
                      type="button"
                      onClick={() => addOpening(side.id, "window")}
                      className="rounded-xl border border-black px-4 py-2"
                    >
                      + Extra kozijnmaat
                    </button>
                    <button
                      type="button"
                      onClick={() => addOpening(side.id, "door")}
                      className="rounded-xl border border-black px-4 py-2"
                    >
                      + Extra deurmaat
                    </button>
                    <button
                      type="button"
                      onClick={() => addOpening(side.id, "other")}
                      className="rounded-xl border border-black px-4 py-2"
                    >
                      + Overige opening
                    </button>
          ...
          {selectedProduct && (
            <div className="mt-4 rounded-xl border border-black p-4">
              ...
              <p className="mt-2 text-sm">
                Prijs: €{selectedProduct.pricePerM2ExVat.toFixed(2)} excl. btw
                per m²
              </p>
              <p className="mt-1 text-sm">
                Snijverlies: {selectedProduct.wasteFactor}%
              </p>
            </div>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-black bg-white p-4">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-black p-3">
              ...
              <div className="text-lg font-semibold">
                {round2(totals.gross).toFixed(2)} m²
              </div>
            </div>
            ...
            <div className="rounded-xl border border-black p-3">
              ...
              <div className="text-lg font-semibold">
                {round2(totals.openings).toFixed(2)} m²
              </div>
            </div>
          ...
          <section className="rounded-2xl border border-black bg-white p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <div className="rounded-xl border border-black p-3">
                ...
                <div className="text-lg font-semibold">
                  {materialResult.netWithWaste.toFixed(2)} m²
                </div>
              </div>
              ...
              <div className="rounded-xl border border-black p-3">
                ...
                <div className="text-lg font-semibold">
                  €{materialResult.materialPriceExVat.toFixed(2)}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-black p-4 text-sm">
              <p>
                Verbindingsprofiel:{" "}
                {materialResult.needsConnectionProfile ? "Ja" : "Nee"}
              </p>
              <p>
                Startprofiel: {materialResult.needsStartProfile ? "Ja" : "Nee"}
              </p>
              <p>
                Beginprofiel indicatie:{" "}
                {materialResult.needsStartProfile
                ? `${round2(totals.gross > 0 ? totals.net : 0)} m² gevel aanwezig - lengteberekening volgt`
              : "Niet nodig"}
              </p>
              <p>
                Eindprofiel: {materialResult.needsEndProfile ? "Ja" : "Nee"}
              </p>
              <p>
                Hoekprofiel: {materialResult.needsCornerProfile ? "Ja" : "Nee"}
              </p>
            </div>
          </section>
        )}
      ...
      <div className="fixed inset-x-0 bottom-0 border-t border-black bg-white p-4">
        <div className="mx-auto flex max-w-6xl gap-3">
          <button
            ...
            className="rounded-xl border border-black px-4 py-3 disabled:opacity-50"
          >
            Zijde toevoegen
          </button>
        </div>
