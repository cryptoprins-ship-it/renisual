...
export default function GevelCalcPage() {
  // #State: hoofdgegevens, productkeuze en totaalprijs-korting
  ...
  // #Foto uploaden: foto wordt opgeslagen als base64 voor JSON export/import en PDF-print
  ...
  // #Actieve zijdes: lege zijdes tellen niet mee
  ...
  // #Materiaalberekening: panelen op m², profielen op meters, korting op totale prijs
  const materialResult = useMemo(() => {
    ...
    const panelCount =
      selectedProduct.type === "panel" && panelAreaM2 > 0
        ? Math.ceil(netWithWaste / panelAreaM2)
        : 0;

    const materialPriceExVat = round2(
      panelCount *
        (selectedProduct.pricePerPanelExVat ??
          panelAreaM2 * selectedProduct.pricePerM2ExVat)
    );
    ...
    const profilePriceExVat = round2(profileItems.reduce((sum, item) => sum + item.totalExVat, 0));
  ...
  }, [selectedProduct, orientation, activeSides, sides, frontBackSame, leftRightSame, totals.net, totalDiscountPercent]);

  // #Opslaan/export/import: inclusief foto's
  ...
  // #PDF export: browser print naar PDF
  function exportPdf() {
    window.print();
  }
...
Profielenprijs excl. btw: €${materialResult?.profilePriceExVat.toFixed(2) ?? "0.00"}
Subtotaal excl. btw: €${materialResult?.subtotalExVat.toFixed(2) ?? "0.00"}
Korting totaalprijs: ${totalDiscountPercent}%
Korting bedrag: €${materialResult?.totalDiscount.toFixed(2) ?? "0.00"}
Totaal excl. btw na korting: €${materialResult?.totalExVat.toFixed(2) ?? "0.00"}
`);
            ...
            return (
              <section key={side.id} className="rounded-2xl border border-black bg-white p-4 md:p-6">
                ...
                <div className="mt-5 rounded-2xl border-2 border-dashed border-black p-4 text-center print-avoid">
                  ...
                  {side.previewUrl && <img src={side.previewUrl} alt={side.name} className="mx-auto mt-4 max-h-[320px] w-full rounded-xl object-contain" />}
                </div>
          ...
          {selectedProduct && materialResult && (
            <section className="rounded-2xl border border-black bg-white p-4 print-avoid">
              ...
              <div className="mt-4 rounded-xl border border-black p-4 text-sm">
                <p>Panelenprijs excl. btw: €{materialResult.materialPriceExVat.toFixed(2)}</p>
                <p>Profielenprijs excl. btw: €{materialResult.profilePriceExVat.toFixed(2)}</p>
                <p>Subtotaal excl. btw: €{materialResult.subtotalExVat.toFixed(2)}</p>
                <p>Korting totaalprijs: {totalDiscountPercent}%</p>
                <p>Korting bedrag: €{materialResult.totalDiscount.toFixed(2)}</p>
                <p className="font-semibold">Totaal excl. btw na korting: €{materialResult.totalExVat.toFixed(2)}</p>
              </div>
            </section>
          )}
        </div>
