...
function buildStylePrompt(opts: ResolvePromptOpts): string {
  ...
  if (opts.shape === "brick") {
    ...
    surfaceDescriptor = "cladding panels matching the reference image (image 2) EXACTLY in colour, lightness, and surface character. The reference image is the AUTHORITATIVE colour source — match its exact lightness, hue, and saturation, NOT the average wood colour from training data. If the reference shows weathered grey-brown driftwood, render LIGHT weathered grey-brown driftwood (muted, faded, slightly silvery — NOT dark walnut, NOT espresso, NOT chocolate). If the reference shows grey wood-grain, render grey wood-grain. If it shows oak, render oak. If it shows a smooth solid colour, render smooth solid panels in that exact colour. The reference dictates the lightness — do NOT darken, do NOT default to honey-brown, pine, walnut, espresso, or chocolate wood. Flat panels with very slight surface relief from the print, NOT truly grooved planks.";
  }
